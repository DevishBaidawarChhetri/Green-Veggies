                                ...
                                <tbody>
                                        ...
                                        <td>
                                            <strong class="text-capitalize">
                                                @if($order->status === 'processing')
                                                    <p class="badge badge-warning bg-orange" style=" border-radius: 0; color: #fff !important;  border-radius: 0;">{{$order->status}}</p>
                                                @endif
                                                @if($order->status === 'order confirmed')
                                                    <p class="badge badge-info" style="border-radius: 0;">Order Confirmed</p>
                                                @endif
                                                @if($order->status === 'shipped')
                                                    <p class="badge badge-secondary" style="border-radius: 0;">Shipped</p>
                                                @endif
                                                @if($order->status === 'delivered')
                                                    <p class="badge badge-success" style="border-radius: 0;">Delivered</p>
                                                @endif
                                            </strong>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        @if($order->status ==  'order confirmed' || $order->status == 'shipped')
                                            <th scope="row" colspan="10">
                                                <div class="tooltip-wrapper">
                                                    <button class="btn btn-danger float-right tooltip" style="border: none; border-radius: 0;" disabled="disabled">
                                        ...
                                        @elseif($order->status == 'delivered')
                                            <th scope="row" colspan="10">
                                                <a href="{{url('/viewBill')}}/{{$order->id}}" class="btn btn-success float-right" style="border: none; border-radius: 0;"><i class="fas fa-receipt"></i>&nbsp;&nbsp;View Bill</a>
                                            </th>
                                        @else
                                            <th scope="row" colspan="10">
                                                <a href="{{url('orderProduct')}}/{{$order->id}}" class="btn btn-danger float-right" style="border: none; border-radius: 0;" onclick="return confirm ('Are you sure you want to cancel your order?');"><i class="fas fa-times"></i>&nbsp;&nbsp;Cancel Order</a>
                                            </th>
                                        @endif
                                    </tr>
                                </tbody>
